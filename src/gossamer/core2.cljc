...
(defmulti update-host-container (fn [host-config _ _] (host-config-type host-config)))

(defmethod update-host-container ::mutation
...
(defn append-all-children-to-container
  ...
  (let [node (some-> fiber-ref deref :child)]
    (loop [node node]
      (when node
        (cond
          (= (:tag node) :HostComponent)
            (let [instance (:state-node node)]
...
(defmethod update-host-container ::persistence
  [host-config context-ref fiber-ref]
  {:pre [@context-ref]
   :post [@context-ref]}
  ;(log/trace "update-host-component:\n" (-> fiber-ref deref pp))
  (log/info "update-host-container" host-config)
...
(defmethod update-host-component ::persistence
  ...
  (let [type (some-> fiber-ref deref :type)
    ...
    (ghc/finalize-initial-children
      ...
      ;currentHostContext
      nil))
  (some-> context-ref deref :wip-fiber-ref deref :child))
...
(defn begin-work
  ...
  (let [isFunctionComponent (fn? (-> fiber-ref deref :type))
    ...
    (cond
      ...
      isHostRoot
        (update-host-container host-config context-ref fiber-ref)
      :else
...
(defn perform-unit-of-work
  ...
  (if-let [next (begin-work host-config context-ref unit-of-work)]
    next
  ...
  (loop [next-unit-of-work-ref (some-> context-ref deref :next-unit-of-work-ref)
    ...
    (when  (and (some-> next-unit-of-work-ref deref)
      ...
      (let [uow (perform-unit-of-work
        ...
        (recur uow true #_(< (time-remaining-fn) 1))))))
    ...
    (go-loop []
      (let [wip-root-ref (<! work-chan)]
        ...
        (let [; FIXME use async timeouts
              time-remaining 10
              next-unit-of-work-ref (work-loop-concurrent host-config context-ref time-remaining)]
